...
public class personaDAO {
    ...
    public String insertpersona(persona pers){
        ...
        try {
            ...
            con.ps.setString(14,pers.getGen_cliente());
            con.ps.setString(15,pers.getCod_sucursal());
            //con.ps.execute();
            //con.ps.close();
...
//            con.ps.close();
    ...
    public List<persona> listpersona(String sucur){
        List<persona> listapersona = new ArrayList<>();
        persona person;
        try {
            PreparedStatement conex = con.conectar_bd().prepareStatement("SELECT v.cod_persona,v.nombre,v.ruc_ci,v.direccion,v.celular,v.telefono_domicilio FROM view_persona v WHERE v.cod_sucursal = '"+sucur+"'");
    ...
    public List<persona> listbuspernom(String nom,String sucur){
        List<persona> listapersona = new ArrayList<>();
        persona person;
        try {
            PreparedStatement conex = con.conectar_bd().prepareStatement("SELECT v.cod_persona,v.nombre,v.ruc_ci,v.direccion,v.celular,v.telefono_domicilio FROM view_persona v WHERE v.cod_sucursal = '"+sucur+"' and v.nombre LIKE '"+nom+"' ");
            ResultSet result = conex.executeQuery();
            while(result.next()){
                 person= new persona();
                 person.setCod_persona(result.getInt(1));
    ...
    public List<persona> listbuspernrodoc(String nom,String sucur){
        List<persona> listapersona = new ArrayList<>();
        persona person;
        try {
            PreparedStatement conex = con.conectar_bd().prepareStatement("SELECT v.cod_persona,v.nombre,v.ruc_ci,v.direccion,v.celular,v.telefono_domicilio FROM view_persona v WHERE v.cod_sucursal = '"+sucur+"' and v.ruc_ci='"+nom+"' ");
            ResultSet result = conex.executeQuery();
            while(result.next()){
                 person= new persona();
                 person.setCod_persona(result.getInt(1));
    ...
    public List<persona> listapersonaedit(String codigo,String sucur){
        List<persona> listapersona = new ArrayList<>();
        persona person;
        try {
            PreparedStatement conex = con.conectar_bd().prepareStatement("SELECT * FROM view_persona d WHERE d.cod_sucursal = '"+sucur+"' and d.cod_persona='"+codigo+"'");
            ResultSet result = conex.executeQuery();
            while(result.next()){
                 person= new persona();
                 person.setCod_persona(result.getInt(1));
                 ...
                 person.setGen_cliente(result.getString(19));
                 listapersona.add(person);
            }
           // con.closeConnection();
        } catch (SQLException e) {
            System.out.println("Error al retornar datos de la persona: "+e);
        }
        return listapersona;
    }
    ...
    public String retorna_nomb_persona(String valor,String sucur){
       String desc=null;
        try {
            con.ps = con.conectar_bd().prepareStatement("SELECT p.nombre FROM persona p WHERE p.cod_sucursal = '"+sucur+"' and p.cod_persona='"+valor+"'");
            con.rs=con.ps.executeQuery();
            con.rs.next();
