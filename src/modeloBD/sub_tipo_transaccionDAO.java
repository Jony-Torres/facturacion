...
public class sub_tipo_transaccionDAO {
    ...
    public Integer valida_sub_tipo_trans(String mod,Integer valor1){
       Integer cod = null;
        try {
            con.ps = con.conectar_bd().prepareStatement("SELECT sub.cod_tipo_trans FROM sub_tipo_transaccion sub WHERE sub.cod_modulo = '"+mod+"' and sub.cod_sub_tipo_trans = '"+valor1+"'");
            con.rs=con.ps.executeQuery();
            con.rs.next();
            cod = con.rs.getInt("cod_tipo_trans");
            //con.closeConnection();
