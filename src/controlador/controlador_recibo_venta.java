...
public void cargar_recibo_venta(JTable tbl_comprobante,String opc){
    ...
    public void actionPerformed(ActionEvent e) {
            ...
            if (vistaRecibo_ven.txt_cod_cliente.getText().equals("")) {
                ...
                if(res!= null){
                    ...
                    String resp=modeloCliente.consulta_existencia_saldo(Integer.parseInt(vistaRecibo_ven.txt_cod_cliente.getText()));
                        if(resp.equals("S")){
                            carga_dato_cliente();
                            vistaRecibo_ven.txt_sub_tipo_transaccion.setText("2");
        ...
        if(e.getSource()== vistaRecibo_ven.btn_error){
            ...
            if (vistaRecibo_ven.txt_total_comprobante.getText().equals("")) {
                JOptionPane.showMessageDialog(null,"No hay valores para la impresion del Recibo","Mensaje del sistema",JOptionPane.INFORMATION_MESSAGE);
            } else {
                imprimir_recibo();
            }
        }
    ...
    private void cargar_saldo_cliente(JTable tbl_saldo_cliente) {
        ...
        List<saldo_cliente> saldoList = modeloSaldo.listdesaldo(vistaRecibo_ven.txt_cod_cliente.getText(),sucursal);
            ...
            if (rptregis!="Registro exitoso.") {
                ...
                while (i < fselect) {
                    ...
                    if (estadomov == true) {
                        ...
                        int opc = JOptionPane.showConfirmDialog(null,"Desea Imprimir el Recibo?", "Mensaje Del Sistema", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
                        if (opc == JOptionPane.YES_OPTION) {
                            imprimir_recibo();
                        }
                    ...
                    int cod =vistaRecibo_ven.tbl_saldo_cliente.getSelectedRow();
                        ...
                        int[] filasSelec = vistaRecibo_ven.tbl_saldo_cliente.getSelectedRows();
                            if (vistaRecibo_ven.txt_sub_tipo_transaccion.getText().equals("1")) {
                                ...
                                if (ax > 0) {
                                    if (ax > conv_form_num.retorna_valor_numero(vistaRecibo_ven.tbl_saldo_cliente.getValueAt(filasSelec[f],6).toString())) {
    ...
    private void imprimir_recibo() {
        ...
        try {
            try {
                ...
                ruta_informe = "src//reportes//RECIBOMAST.jasper";
                Map parametros = new HashMap();
                
                //CAPTURA TIPO DE COMPROBANTE
                if (vistaRecibo_ven.txt_nro_comp.getText().equals("")) {
                    parametros.put("P_NRO_RECIBO", null);
                } else {
                    parametros.put("P_NRO_RECIBO", Integer.parseInt(vistaRecibo_ven.txt_nro_comp.getText()));
                }
                total = conv_form_num.retorna_valor_numero(vistaRecibo_ven.txt_total_comprobante.getText());
                total_str = String.valueOf(total);
                total_letra = conv_num_let.cantidadConLetra(total_str);
                
                parametros.put("P_TOTAL_NUM", total);
                parametros.put("P_TOTAL_LET",total_letra);
