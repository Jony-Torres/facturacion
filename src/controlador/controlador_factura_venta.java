...
public class controlador_factura_venta extends FORMFACTUR implements ActionListener,KeyListener,FocusListener {
    ...
    public controlador_factura_venta(FORMFACTUR vistaFactura_ven,factura_ventaDAO modeloFactura_ven) {
        ...
        this.vistaFactura_ven.txt_descuento.addActionListener(this);
   ...
   public void cargar_tabla_factura_venta(JTable tbl_comprobante,String us,String suc,String gp,String mod,String form){
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(e.getSource()== vistaFactura_ven.btn_guardar){
            ...
            /*}else if(vistaFactura_ven.txt_comentario.getText().equals("")){
                JOptionPane.showMessageDialog(this, "Debe ingresar la Referencia para el comprobante", "Abvertencia", JOptionPane.WARNING_MESSAGE);
        ...
        /*if(e.getSource()== vistaFactura_ven.btn_pagr){
            if (vistaFactura_ven.txt_total.getText().equals("")) {
                JOptionPane.showMessageDialog(null,"No hay valores para la impresion del Comprobante","Mensaje del sistema",JOptionPane.INFORMATION_MESSAGE);
            } else {
                imprimir_comprobante(vistaFactura_ven.cmb_tipo_comprobante.getSelectedItem().toString(),
                                    vistaFactura_ven.txt_ser_comprobante.getText(),
                                    Integer.parseInt(vistaFactura_ven.txt_nro_comprobante.getText()),
                                    vistaFactura_ven.txt_cod_sucursal.getText());
            }
        }*/
    ...
    private void cargar_cliente(JTable tbl_busq_cliente,String opc) {
        ...
        if(opc == "TODOS") {
            List<cliente> clienteList = modeloCliente.listcliente_LOV(sucursal);
        ...
        if(opc == "CODIGO") {
            ...
            List<cliente> clienteList = modeloCliente.listclientenrodoc_LOV(clie,sucursal);
            for (cliente rpc : clienteList) {
                columna[0] = rpc.getCod_cliente();
                columna[1] = rpc.getDesc_persona();
                model.addRow(columna);
        ...
        if(opc == "NOMBRE") {
            ...
            List<cliente> clienteList = modeloCliente.listclientenomb_LOV(clie,sucursal);
            for (cliente rpc : clienteList) {
                columna[0] = rpc.getCod_cliente();
                columna[1] = rpc.getDesc_persona();
                model.addRow(columna);
    ...
    private void agregar_detalle() {
        ...
        double x = 0.0,
                ...
                descto = 0.0,
                cantpr = 0.0;
        if (vistaFactura_ven.txt_cod_prod_busq.getText() != null && vistaFactura_ven.txt_cod_prod_busq.getText().compareToIgnoreCase("") != 0) {
            if (vistaFactura_ven.txt_precio_unitario.getText().equals("")) {
                JOptionPane.showMessageDialog(this, "Campo Obligatorio debe Ingresar el Precio para continuar", "Abvertencia", JOptionPane.WARNING_MESSAGE);
                vistaFactura_ven.txt_precio_unitario.requestFocus();
                ...
                if (man_st.equals("S")) {
                    ...
                    if (stk >= cantpr) {
                        ...
                        Object filaelementos[] = {codpr, codun, nrolot, cantpr, descpr, prec,formatea.format(descto) , formatea.format(iva5), formatea.format(iva10), formatea.format(grav5), formatea.format(grav10),impo};
                        model.addRow(filaelementos);
                        calcula = importe;//(precio * cantpr);
                        total   = total + calcula;
                        dto     = dto + descto;
                        ...
                        vistaFactura_ven.txt_descuento.setText("");
                        vistaFactura_ven.txt_cod_prod_busq.requestFocus();
                    } else {
                    ...
                    //System.out.println("Cantidad2: "+ cantpr);
                    importe = ((precio * cantpr) - descto);
                    //CALCULO PARA EL IVA 5%
                    if (tiiv == 5) {
                        calcula_iva civ = new calcula_iva(importe);
    ...
    private void quitar_articulo() {
        ...
        if (bselect == -1) {
            ...
            /*desc = vistaFactura_ven.tbl_comprobante.getValueAt(bselect, 6).toString();
            imp  = vistaFactura_ven.tbl_comprobante.getValueAt(bselect, 7).toString();
            iv5  = vistaFactura_ven.tbl_comprobante.getValueAt(bselect, 8).toString();
            iv10 = vistaFactura_ven.tbl_comprobante.getValueAt(bselect, 9).toString(); 
            g5   = vistaFactura_ven.tbl_comprobante.getValueAt(bselect, 10).toString();
            g10  = vistaFactura_ven.tbl_comprobante.getValueAt(bselect, 11).toString();*/
            //
            desto = conv_form_num.retorna_valor_numero(vistaFactura_ven.tbl_comprobante.getValueAt(bselect, 6).toString());
            //
            ...
            //
            imprt = conv_form_num.retorna_valor_numero(vistaFactura_ven.tbl_comprobante.getValueAt(bselect, 11).toString());
            //
            ...
            //System.out.println(" Valores: "+ total + " - "+ desto + " - " + imprt + " - " + i5 + " - " + i10 + " - " + gr5 + " - " + gr10);
            c    = total - imprt;
            total   = c;
            cdto    = dto - desto;
            dto     = cdto;
    ...
    private void guardar_cambios(String ope) {
        ...
        //DTO
        if (dto == 0.0) {
            fact.setTotal_descuento(0.0);
        } else {
            fact.setTotal_descuento(dto);
        }
        //
        fact.setEstado("P");
        //
            ...
            if (rptregis != "Registro exitoso.") {
                ...
                int fselect = vistaFactura_ven.tbl_comprobante.getRowCount();
                    while (i < fselect) {
                        ...
                        fact.setDescuento(conv_form_num.retorna_valor_numero(vistaFactura_ven.tbl_comprobante.getValueAt(i,6).toString()));
                        ...
                        fact.setImporte_total(conv_form_num.retorna_valor_numero(vistaFactura_ven.tbl_comprobante.getValueAt(i,11).toString()));
                        //
                    ...
                    if (estadomov == true) {
                        ...
                        int opc = JOptionPane.showConfirmDialog(null,"Desea Imprimir el Ticket?", "Mensaje Del Sistema", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
                        if (opc == JOptionPane.YES_OPTION) {
                            
                            if (imprimir_comprobante(fact.getTipo_comprobante(),
                                                    fact.getSer_comprobante(),
                                                    fact.getNro_comprobante(),
                                                    fact.getCod_sucursal()) == true) {
                                /*//Imprime copia
                                imprimir_comprobante(fact.getTipo_comprobante(),
                                                    fact.getSer_comprobante(),
                                                    fact.getNro_comprobante(),
                                                    fact.getCod_sucursal());*/
                            }
        ...
        int cod = vistaFactura_ven.tbl_art_un_lote.getSelectedRow();
                        if (cod==-1) {
                           JOptionPane.showMessageDialog(null, "Debe Seleccionar Una Fila Para Poder Realizar La Operacion  ", "Mensaje Del Sistema", JOptionPane.WARNING_MESSAGE);
                            ...
                            vistaFactura_ven.txt_cod_art_busq.requestFocus();
                        }
    }
    public boolean imprimir_comprobante(String tipo_comp,String ser_comp,Integer nro_comp,String suc){
        ...
        try {
            try {
                ...
                ruta_informe = "src//reportes//REPORT_COMP_VENTA.jasper";
                Map parametros = new HashMap();

                //CAPTURA TIPO DE COMPROBANTE
                /*if (tipo_comp.equals("FACTURA CONTADO")) {
                    tipo_comp = "FCO";
                    parametros.put("p_tipo_comprobante", tipo_comp);
                }
                if (tipo_comp.equals("FACTURA CREDITO")) {
                    tipo_comp = "FCR";
                    parametros.put("p_tipo_comprobante", tipo_comp);
                }*/
                if (tipo_comp.equals("")) {
                    parametros.put("p_tipo_comprobante", null);
                } else {
                   parametros.put("p_tipo_comprobante", tipo_comp); 
                }
                if (ser_comp.equals("")) {
                    parametros.put("p_ser_comprobante", null);
                } else {
                    parametros.put("p_ser_comprobante",ser_comp);
                }

                if (nro_comp.equals("")) {
                    parametros.put("p_nro_comprobante", null);
                } else {
                    parametros.put("p_nro_comprobante", nro_comp);
                }
                ...
                if (suc.equals("")) {
                    ...
                    parametros.put("p_cod_sucursal", suc);
                }
    ...
    limpiar_tabla_comprobante();
        for (factura_venta rpc : factList) {
            ...
            columna[6] =formatea.format(rpc.getDescuento());
            ...
            columna[11] =formatea.format(rpc.getImporte_total());
            model.addRow(columna);
    ...
    private void opc_nuevo_reg() {
        ...
        //Carga cliente por defecto con valor 1
        vistaFactura_ven.txt_cod_cliente.setText(""+1);
        String res = modeloCliente.retorna_desc_cliente(Integer.parseInt(vistaFactura_ven.txt_cod_cliente.getText()));
        if(res!= null){
            vistaFactura_ven.txt_desc_cliente.setText(res);
            carga_dato_cliente();
    ...
    public void habilita_campos() {
        vistaFactura_ven.txt_cantidad.setEditable(true);
        vistaFactura_ven.txt_cod_cliente.setEditable(true);
        vistaFactura_ven.txt_cod_cobrador.setEditable(true);
        vistaFactura_ven.txt_cod_condicion.setEditable(true);
        vistaFactura_ven.txt_cod_moneda.setEditable(true);
        vistaFactura_ven.txt_cod_prod_busq.setEditable(true);
        vistaFactura_ven.txt_cod_sucursal.setEditable(true);
        vistaFactura_ven.txt_cod_deposito.setEditable(true);
        vistaFactura_ven.txt_cod_vendedor.setEditable(true);
        vistaFactura_ven.txt_desc_cliente.setEditable(true);
        vistaFactura_ven.txt_desc_cobrador.setEditable(true);
        vistaFactura_ven.txt_desc_condicion.setEditable(true);
        vistaFactura_ven.txt_desc_moneda.setEditable(true);
        vistaFactura_ven.txt_desc_prod_busq.setEditable(true);
        vistaFactura_ven.txt_desc_sucursal.setEditable(true);
        vistaFactura_ven.txt_desc_vendedor.setEditable(true);
        vistaFactura_ven.txt_fecha_comprobante.setEnabled(true);
        ...
        if (act_prec.equals("S")) {
            vistaFactura_ven.txt_precio_unitario.setEditable(true);
        }
        if (act_dto.equals("S")) {
            vistaFactura_ven.txt_descuento.setEditable(true);
        }
        vistaFactura_ven.txt_comentario.setEditable(true);
        //txt_total.setEditable(true);
        //txt_total_gravada.setEditable(true);
        //txt_total_iva.setEditable(true);
        /*botones de busqueda*/
        vistaFactura_ven.btn_suc.setEnabled(true);
        vistaFactura_ven.btn_dep.setEnabled(true);
        vistaFactura_ven.btn_client.setEnabled(true);
        vistaFactura_ven.btn_serie.setEnabled(true);
        vistaFactura_ven.btn_cob.setEnabled(true);
        vistaFactura_ven.btn_ven.setEnabled(true);
        vistaFactura_ven.btn_cond.setEnabled(true);
        vistaFactura_ven.btn_mon.setEnabled(true);
        vistaFactura_ven.btn_prod.setEnabled(true);
        vistaFactura_ven.txt_cod_cliente.requestFocus();
    }
    private void carga_tabla_comprobante(JTable tbl_comprobante,String opc) {
        ...
        int[] anchos = {80,15,15,15,250,50,40,20,40,20,50,50};
    ...
    private void valida_codigo_articulo() {
        boolean estado = false;
        Integer precio_venta = 0;
        ...
        //System.out.println("Valor: "+res1);
        if (res1 == 1 /*|| ((res1 > 1 && ((vistaFactura_ven.txt_cod_un_med.getText() != null && vistaFactura_ven.txt_cod_un_med.getText().compareToIgnoreCase("") != 0) &&
                                        (vistaFactura_ven.txt_nro_lote != null && vistaFactura_ven.txt_nro_lote.getText().compareToIgnoreCase("") != 0))))*/) {
            ...
            if (estado == true) {
                precio_venta = modeloArticulo.retorna_precioun_art(vistaFactura_ven.txt_cod_prod_busq.getText(), vistaFactura_ven.txt_cod_un_med.getText(), vistaFactura_ven.txt_nro_lote.getText());
                if (vistaFactura_ven.txt_precio_unitario.getText() == null || vistaFactura_ven.txt_precio_unitario.getText().compareToIgnoreCase("") == 0) {    
                    vistaFactura_ven.txt_precio_unitario.setText("" + precio_venta);
                }
                //if (vistaFactura_ven.txt_cantidad.getText() == null && vistaFactura_ven.txt_cantidad.getText().compareToIgnoreCase("") == 0) {
                if (vistaFactura_ven.txt_cantidad.getText() == null || vistaFactura_ven.txt_cantidad.getText().compareToIgnoreCase("") == 0) {    
        ...
        } else if (res1 > 1 && ((vistaFactura_ven.txt_cod_un_med.getText() != null && vistaFactura_ven.txt_cod_un_med.getText().compareToIgnoreCase("") != 0) &&
                                (vistaFactura_ven.txt_nro_lote != null && vistaFactura_ven.txt_nro_lote.getText().compareToIgnoreCase("") != 0))) {
            List<articulo> articuloList = modeloArticulo.dato_art_un_lot(vistaFactura_ven.txt_cod_prod_busq.getText(), vistaFactura_ven.txt_cod_un_med.getText(), vistaFactura_ven.txt_nro_lote.getText());
            for (articulo rpc : articuloList) {
                vistaFactura_ven.txt_desc_prod_busq.setText(rpc.getDesc_articulo());
                vistaFactura_ven.txt_cod_un_med.setText(rpc.getCod_un_med());
                vistaFactura_ven.txt_nro_lote.setText(rpc.getNro_lote());
