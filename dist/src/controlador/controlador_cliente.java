...
public class controlador_cliente extends FORMCLIENT implements ActionListener,KeyListener,MouseListener{
    ...
    public controlador_cliente(FORMCLIENT vistaCliente,clienteDAO modeloCliente,String gp,String mod,String form,String sucur) {
    ...
    public void cargar_clientes(JTable tbl_cliente,String opc){
        ...
        if (opc == "TODOS") {
            List<cliente> clienteList = modeloCliente.listcliente(sucursal);
        ...
        if (opc == "COD_PERSONA") {
            List<cliente> clienteList = modeloCliente.listclientenrodoc(vistaCliente.txt_cod_persona.getText(),sucursal);
                for (cliente rpc : clienteList) {
                    columna[0] = rpc.getCod_cliente();
                    columna[1] = rpc.getDesc_persona();
                    columna[2] = rpc.getDesc_condicion();
        ...
        if (opc == "COD_CLIENTE") {
            List<cliente> clienteList = modeloCliente.listclientcod(vistaCliente.txt_cod_cliente.getText(),sucursal);
                for (cliente rpc : clienteList) {
                    columna[0] = rpc.getCod_cliente();
                    columna[1] = rpc.getDesc_persona();
                    columna[2] = rpc.getDesc_condicion();
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(e.getSource()== vistaCliente.txt_cod_persona){
            if (vistaCliente.txt_cod_persona.getText().equals("")) {
                ...
                String res = modeloPerson.retorna_nomb_persona(vistaCliente.txt_cod_persona.getText(),sucursal);
                if(res!= null){
        ...
        if(e.getSource()== vistaCliente.btn_persona){
            ...
            controlador_persona ctrlper = new controlador_persona(vistap,per,grupo,"CC","FORMPERSON",sucursal);
    ...
    private void cargar_persona(JTable tbl_bpersona,String opc) {
        ...
        if (opc == "TODOS") {
            List<persona> personaList = modeloPerson.listpersona(sucursal);
        ...
        if (opc == "CODIGO") {
            ...
            List<persona> personaList = modeloPerson.listbuspernrodoc(per,sucursal);
            for (persona rpc : personaList) {
                columna[0] = rpc.getCod_persona();
                columna[1] = rpc.getNombre();
                model.addRow(columna);
        ...
        if (opc == "NOMBRE") {
            ...
            List<persona> personaList = modeloPerson.listbuspernom(per,sucursal);
            for (persona rpc : personaList) {
                columna[0] = rpc.getCod_persona();
                columna[1] = rpc.getNombre();
                model.addRow(columna);
    ...
    private void guardar_cambios(String ope) {
        ...
        }else if(modeloPerson.retorna_nomb_persona(vistaCliente.txt_cod_persona.getText(),sucursal) == null) {
               ...
               int cod =vistaCliente.tbl_cliente.getSelectedRow();
                        if (cod==-1) {
                           JOptionPane.showMessageDialog(null, "Debe Seleccionar Una Fila Para Poder Realizar La Operacion  ", "Mensaje Del Sistema", JOptionPane.WARNING_MESSAGE);
                            ...
                            List<cliente> clienteList = modeloCliente.listaclientedit(vistaCliente.tbl_cliente.getValueAt(cod, 0).toString(),sucursal);
                            for (cliente rpc : clienteList) {
