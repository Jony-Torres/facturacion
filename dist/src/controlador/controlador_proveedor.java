...
public class controlador_proveedor extends FORMPROVEE implements ActionListener,KeyListener,MouseListener {
    ...
    String permiso              =null;
    String grupo                = null;
    public controlador_proveedor(FORMPROVEE vistaProveedor,proveedorDAO modeloProveedor,String gp,String mod,String form,String sucur) {
        this.grupo=gp;
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() == vistaProveedor.txt_cod_persona) {
            if (vistaProveedor.txt_cod_persona.getText().isEmpty()) {
                ...
                String res = modeloPersona.retorna_nomb_persona(vistaProveedor.txt_cod_persona.getText(),sucursal);
                if(res!= null){
    ...
    private void cargar_datos_persona(JTable tbl_busq_pers,String op) {
        ...
        if (op == "TODOS") {
            List<persona> persList = modeloPersona.listpersona(sucursal);
        ...
        if (op == "CODIGO") {
            List<persona> persList = modeloPersona.listbuspernrodoc(vistaProveedor.txt_busq_cod_pers.getText(),sucursal);
            for (persona rpc : persList) {
                columna[0] = rpc.getCod_persona();
                columna[1] = rpc.getNombre();
                model.addRow(columna);
        ...
        if (op == "NOMBRE") {
            List<persona> persList = modeloPersona.listbuspernom(vistaProveedor.txt_busq_nomb_pers.getText(),sucursal);
            for (persona rpc : persList) {
                columna[0] = rpc.getCod_persona();
                columna[1] = rpc.getNombre();
                model.addRow(columna);
    ...
    private void guardar_cambios(String ope) {
        ...
        if (vistaProveedor.txt_cod_persona.getText().equals("")) {
            ...
            if(modeloPersona.retorna_nomb_persona(vistaProveedor.txt_cod_persona.getText(),sucursal) == null) {
