...
public class controlador_cons_saldo_cliente extends FORMCOSACL implements ActionListener,KeyListener{
    ...
    String sucursal;
    public controlador_cons_saldo_cliente(FORMCOSACL vistaCons_saldo_cliente,saldo_clienteDAO modeloCons_saldo_cliente,String sucur) {
    ...
    public void cargar_saldos_clientes(JTable tbl_saldo_cliente,String opc){
        ...
        if(opc == "TODOS") {
            List<saldo_cliente> saldList = modeloCons_saldo_cliente.listdesaldocliente(sucursal);
            ...
            vistaCons_saldo_cliente.txt_total.setText(""+formatea.format(modeloCons_saldo_cliente.retorna_saldo_tot_cliente(sucursal)));
        }
        if(opc == "CODIGO") {
            List<saldo_cliente> saldList = modeloCons_saldo_cliente.listdecliente_codigo(vistaCons_saldo_cliente.txt_cod_cliente.getText(),sucursal);
            for (saldo_cliente rpc : saldList) {
                columna[0] = rpc.getTipo_comprobante();
                columna[1] = rpc.getSer_comprobante();
                columna[2] = rpc.getNro_comprobante();
        ...
        if(opc == "FECHA_VENC") {
            ...
            String fechcomp2 = formatofecha_mysql.format(fchi2);
                List<saldo_cliente> saldList = modeloCons_saldo_cliente.listdecliente_fec_ven(fechcomp1,fechcomp2,sucursal);
                for (saldo_cliente rpc : saldList) {
                    columna[0] = rpc.getTipo_comprobante();
                    columna[1] = rpc.getSer_comprobante();
                    columna[2] = rpc.getNro_comprobante();
        ...
        if(opc == "FECHA_VENC_CODIGO"){
            ...
            String codigo = vistaCons_saldo_cliente.txt_cod_cliente.getText();
                List<saldo_cliente> saldList = modeloCons_saldo_cliente.listdecliente_cod_fec_ven(codigo,fechcomp1,fechcomp2,sucursal);
                for (saldo_cliente rpc : saldList) {
                    columna[0] = rpc.getTipo_comprobante();
                    columna[1] = rpc.getSer_comprobante();
                    columna[2] = rpc.getNro_comprobante();
    ...
    private void cargar_cliente(JTable tbl_busqueda,String opc) {
        ...
        if (opc == "RUC_CI") {
            List<cliente> clienteList = modeloCliente.listclientenrodoc_LOV(vistaCons_saldo_cliente.txt_bnro_documento.getText(),sucursal);
            for (cliente rpc : clienteList) {
                columna[0] = rpc.getCod_cliente();
                columna[1] = rpc.getDesc_persona();
                model.addRow(columna);
        ...
        if (opc == "NOMBRE") {
            List<cliente> clienteList = modeloCliente.listclientenomb_LOV(vistaCons_saldo_cliente.txt_bnomb_cliente.getText(),sucursal);
            for (cliente rpc : clienteList) {
                columna[0] = rpc.getCod_cliente();
                columna[1] = rpc.getDesc_persona();
                model.addRow(columna);
